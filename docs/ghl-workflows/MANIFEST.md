...
> **2026-04-30 update:** The 4 BUILT workflows (#6, #10, #11, #14) now have drop-in paste sheets at [`paste-sheets/`](./paste-sheets/) — generated by `website/scripts/render-ghl-paste-sheets.ts`. A human with GHL 2FA access can wire all 4 in ~45-60 min. The other 10 workflows remain blocked on Elliott copy approval (see "Recommended Linear Sub-Tasks" at the bottom). Re-run the renderer after Elliott copy lands in `website/src/lib/email-templates.ts`.
